...
$(function (){
	//设置注销事件方法
	 $("#Loginout").on('click', function () {
		//执行注销的方法
		 $.post("/MyWeb/LoginController/loginout.do",function(data){
				console.log(data);
				alert("请先登陆");
	...
	//返回的数据为有已发布文章的页码
	$.post("/MyWeb/Issue/AllpageByIssue.do",function(data){//如果没有登陆的事件处理方法
		if(data.count==-1){
			alert("请先登陆");
			window.open("passage.html","_self");
		}else{
	...
	//登陆成功执行该方法
	function load(){
		$.post("/MyWeb/Issue/passageLoad.do",function(data){
		...
		$("#Bgo").click(function(){
			...
			$.post("/MyWeb/Issue/goPage.do",{goPage:Npage},function(data){
				page(data);
		...
		$("#BupPage").click(function(){
			...
			if(up>0){
				$.post("/MyWeb/Issue/upPage.do",{upPage:up},function(data){
					page(data);
				},"json");
				$("#Tpage").val(up);
			}
		...
		$("#BdownPage").click(function(){
			...
			if(down<=allpage){
				$.post("/MyWeb/Issue/downPage.do",{downPage:down},function(data){
					page(data)
	...
	$("#Apublish").on('click', function () {
		...
		if(Atext=="发布"){
			...
			if(textHtml!=""&&Ptitle!=""&&Pclassify!=""&&pbrief!=""){
				...
				$.post("/MyWeb/Issue/publish.do",{Ptitle:Ptitle,pbrief:pbrief,Pclassify:Pclassify,textHtml:textHtml},function(data){	
					if(data=="true"){
						alert("发布成功");
