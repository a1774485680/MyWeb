	...
	$(function (){
		...
		$.post("/MyWeb/Passage/passageLoad.do",function(data){
			var fal=0;//记录是第几个div
		...
		//设置分页的系统 获得当前的行数
		$.post("/MyWeb/Passage/allPage.do",{NowCategory:Nowclassify},function(data){
			allpage=data.count;
		...
		$("#Bgo").click(function(){
			...
			$.post("/MyWeb/Passage/goPage.do",{goPage:Npage,NowCategory:Nowclassify},function(data){
				page(data);
			},"text");
			}else{
				$("#Tpage").val("1");
		...
		$("#BupPage").click(function(){
			...
			if(up>0){
				$.post("/MyWeb/Passage/upPage.do",{upPage:up,NowCategory:Nowclassify},function(data){
					page(data);
				},"text");
				$("#Tpage").val(up);
			}
		...
		$("#BdownPage").click(function(){
			...
			if(down<=allpage){
				$.post("/MyWeb/Passage/downPage.do",{downPage:down,NowCategory:Nowclassify},function(data){
					page(data);
				},"text");
				$("#Tpage").val(down);				
			} 
	...
	function classify(id)
		{		
				...
				falof2=1;//将获得类别文章信息的标志位设置位1，即不允许点击类别
		 		...
		 		$.post("/MyWeb/Passage/classify.do",{NowCategory:Nowclassify},function(data){
		 			page(data);
		 			falof2=0;
		 			if(falof1==0&&falof2==0){			 			
			 			$.each(arr,function(){
